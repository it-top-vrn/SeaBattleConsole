	...
	if (viborSea == 1) // Выбор меню играть
	{
		while (true)
		{
			...
			if (ship == 11 || ship == 12)
			{
				...
				if (user == 0)
				{
					selectionEnd_1 = "\t\t1 - срыть корабли и сменить игрока";
				}
				else if (user == 1)
				{
					selectionEnd_1 = "\t\t1 - срыть корабли и перейти к стрельбе";
				}
...
void OkrasField(char map[][19], int color1, int color2, int color3, int color4, int color5, int color6, int x)
{
	...
	for (int i = 0; i < 17; i++) //печать поля
	{
		...
		for (int j = 0; j < 19; j++)
		{
			if (map[i][j] == '.')
			{
				//Перед строкой для выделения текста цветом :
...
void eliminationSticking(char map[][19])
{
	if (map[y][x] == '@') // устранение залипания курсора на первом поле
	{
		map[y][x] = ' ';
	}
}
...
void ConsoleWindow()
{
	...
	SetWindowPos(window_header, HWND_TOP, 200, 100, 600, 1000, NULL);
}
